...
static int nvme_tcp_try_send_cmd_pdu(struct nvme_tcp_request *req)
{
	...
	iov_iter_bvec(&msg.msg_iter, ITER_SOURCE, &bvec, 1, len);
	printk("you are about to send out a cmd pdu using Homa!\n");
	ret = sock_sendmsg(queue->sock, &msg);
	if (ret < 0)
		return ret;
	...
	/* This part is of no interest for Homa, as it is message-based. */
	// len -= ret;
	// if (!len) {
	// 	if (inline_data) {
...
static int nvme_tcp_try_send(struct nvme_tcp_queue *queue)
{
	...
	if (req->state == NVME_TCP_SEND_CMD_PDU) {
		printk("You are sending a cmd_pdu.\n");
		ret = nvme_tcp_try_send_cmd_pdu(req);
		if (ret <= 0)
			goto done;
		printk("The cmd pdu successfully sent.");
				...
				key_serial_t pskid)
{
	...
	/* Set TCP no delay, TCP specific, not interested for Homa */
	 ...
	 * TCP specific, not interested for Homa.
	 */
	 ...
	 * The following fields are not used by Homa, hence omitted.
	 */
	...
	/* This is not concerned by us for this specific project.*/
	// if (queue->hdr_digest || queue->data_digest) {
	...
	dev_dbg(nctrl->device, "connecting queue %d\n",
			nvme_tcp_queue_id(queue));
	// Already implemented Homa's connect()
	 ...
	 * Homa has nothing to do with TLS, no use.
	 */
...
static struct nvmf_transport_ops nvme_homa_transport = {
	.name		= "homa",
	.module		= THIS_MODULE,
	.required_opts	= NVMF_OPT_TRADDR,
	.allowed_opts	= NVMF_OPT_TRSVCID |
			  NVMF_OPT_HOST_TRADDR |
			  NVMF_OPT_NR_WRITE_QUEUES | NVMF_OPT_CTRL_LOSS_TMO | NVMF_OPT_NR_POLL_QUEUES |
...
static int __init nvme_tcp_init_module(void)
{
	...
	nvme_tcp_wq = alloc_workqueue("nvme_homa_wq", wq_flags, 0);
	if (!nvme_tcp_wq)
		return -ENOMEM;
	...
	nvmf_register_transport(&nvme_homa_transport);
	return 0;
}
...
static void __exit nvme_tcp_cleanup_module(void)
{
	...
	nvmf_unregister_transport(&nvme_homa_transport);
...
MODULE_DESCRIPTION("NVMe host Homa transport driver");
MODULE_LICENSE("GPL v2");
MODULE_VERSION("0.1");
MODULE_ALIAS("nvme_homa");
