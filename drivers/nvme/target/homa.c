...
struct nvmet_homa_queue {
	// This should be a global Homa socket created when add_port is called.
	struct socket		*sock;
	struct nvmet_tcp_port	*port;
	// For Homa, the recv_work is global for a port and for each queue we only
	// care about send_work
	// For now io_work is global, because we only cater for 1 queue
	...
	/* digest state, they should not be used by Homa */
	bool			hdr_digest;
	bool			data_digest;
	struct ahash_request	*snd_hash;
	struct ahash_request	*rcv_hash;

	/* TLS state, also should not be used by Homa */
	/* TODO: DELETE THIS ONCE THE LOGIC SKELETON IS BUILT */
...
struct nvmet_tcp_port {
	struct socket		*sock;
	// Instead of tcp's listen and accept,
	// we implement global io_work
...
static const struct nvmet_fabrics_ops nvmet_homa_ops;
static void nvmet_tcp_free_cmd(struct nvmet_tcp_cmd *c);
static void nvmet_tcp_free_cmd_buffers(struct nvmet_tcp_cmd *cmd);

static inline u16 nvmet_tcp_cmd_tag(struct nvmet_homa_queue *queue,
		struct nvmet_tcp_cmd *cmd)
{
...
nvmet_tcp_get_cmd(struct nvmet_homa_queue *queue)
{
	struct nvmet_tcp_cmd *cmd;
...
static inline int queue_cpu(struct nvmet_homa_queue *queue)
{
	return queue->sock->sk->sk_incoming_cpu;
}

static inline u8 nvmet_tcp_hdgst_len(struct nvmet_homa_queue *queue)
{
	return queue->hdr_digest ? NVME_TCP_DIGEST_LENGTH : 0;
}

static inline u8 nvmet_tcp_ddgst_len(struct nvmet_homa_queue *queue)
{
...
static int nvmet_tcp_verify_hdgst(struct nvmet_homa_queue *queue,
	void *pdu, size_t len)
{
...
static int nvmet_tcp_check_ddgst(struct nvmet_homa_queue *queue, void *pdu)
{
	struct nvme_tcp_hdr *hdr = pdu;
...
static void nvmet_tcp_fatal_error(struct nvmet_homa_queue *queue)
{
...
static void nvmet_tcp_socket_error(struct nvmet_homa_queue *queue, int status)
{
	queue->rcv_state = NVMET_TCP_RECV_ERR;
...
static void nvmet_setup_c2h_data_pdu(struct nvmet_tcp_cmd *cmd)
{
	...
	struct nvmet_homa_queue *queue = cmd->queue;
...
static void nvmet_tcp_process_resp_list(struct nvmet_homa_queue *queue)
{
...
static struct nvmet_tcp_cmd *nvmet_tcp_fetch_cmd(struct nvmet_homa_queue *queue)
{
...
static int nvmet_tcp_try_send_one(struct nvmet_homa_queue *queue,
		bool last_in_batch)
{
...
static int nvmet_tcp_try_send(struct nvmet_homa_queue *queue,
...
static void nvmet_prepare_receive_pdu(struct nvmet_homa_queue *queue)
{
	queue->offset = 0;
...
static void nvmet_tcp_free_crypto(struct nvmet_homa_queue *queue)
{
...
static int nvmet_tcp_alloc_crypto(struct nvmet_homa_queue *queue)
{
...
static int nvmet_tcp_handle_icreq(struct nvmet_homa_queue *queue)
{
	struct nvme_tcp_icreq_pdu *icreq = &queue->pdu.icreq;
	struct nvme_tcp_icresp_pdu *icresp = &queue->pdu.icresp;
	// Use this to send icresp, remember to add sendmsg_args
	struct msghdr msg = {};
	struct kvec iov;
	int ret;
...
static void nvmet_tcp_handle_req_failure(struct nvmet_homa_queue *queue,
...
static int nvmet_tcp_handle_h2c_data_pdu(struct nvmet_homa_queue *queue)
{
...
static int nvmet_tcp_done_recv_pdu(struct nvmet_homa_queue *queue)
{
...
static int nvmet_tcp_tls_record_ok(struct nvmet_homa_queue *queue,
...
static int nvmet_tcp_try_recv_pdu(struct nvmet_homa_queue *queue)
{
	struct nvme_tcp_hdr *hdr = &queue->pdu.cmd.hdr;
	int len, ret;
	struct kvec iov;
...
recv:
	// Homa's recvmsg() does not use iov, but we keep it
	// for now, just in case we need it.
	// We need to copy the received pdu from the pool to *pdu.
	iov.iov_base = (void *)&queue->pdu + queue->offset;
	iov.iov_len = queue->left;
	// For Homa we need this for recvmsg_args, also we don't support TLS.
	if (queue->tls_pskid) {
...
static int nvmet_tcp_try_recv_data(struct nvmet_homa_queue *queue)
{
...
static int nvmet_tcp_try_recv_ddgst(struct nvmet_homa_queue *queue)
{
	struct nvmet_tcp_cmd *cmd = queue->cmd;
	int ret, len;
	char cbuf[CMSG_LEN(sizeof(char))] = {};
...
static int nvmet_tcp_try_recv_one(struct nvmet_homa_queue *queue)
{
	int result = 0;
...
static int nvmet_tcp_try_recv(struct nvmet_homa_queue *queue,
...
{
	struct nvmet_homa_queue *queue =
		container_of(kref, struct nvmet_homa_queue, kref);

	WARN_ON(queue->state != NVMET_TCP_Q_DISCONNECTING);
	queue_work(nvmet_wq, &queue->release_work);
}

static void nvmet_tcp_schedule_release_queue(struct nvmet_homa_queue *queue)
{
...
static inline void nvmet_tcp_arm_queue_deadline(struct nvmet_homa_queue *queue)
{
	queue->poll_end = jiffies + usecs_to_jiffies(idle_poll_period_usecs);
}

static bool nvmet_tcp_check_queue_deadline(struct nvmet_homa_queue *queue,
		int ops)
{
...
{
	struct nvmet_homa_queue *queue =
		container_of(w, struct nvmet_homa_queue, io_work);
	bool pending;
	int ret, ops = 0;
...
static int nvmet_tcp_alloc_cmd(struct nvmet_homa_queue *queue,
		struct nvmet_tcp_cmd *c)
{
	...
	c->recv_msg.msg_flags = MSG_DONTWAIT;
...
static int nvmet_tcp_alloc_cmds(struct nvmet_homa_queue *queue)
{
	struct nvmet_tcp_cmd *cmds;
...
static void nvmet_tcp_free_cmds(struct nvmet_homa_queue *queue)
{
...
static void nvmet_tcp_restore_socket_callbacks(struct nvmet_homa_queue *queue)
{
...
static void nvmet_tcp_uninit_data_in_cmds(struct nvmet_homa_queue *queue)
{
...
static void nvmet_tcp_free_cmd_data_in_buffers(struct nvmet_homa_queue *queue)
{
	struct nvmet_tcp_cmd *cmd = queue->cmds;
	int i;
...
{
	struct nvmet_homa_queue *queue =
		container_of(w, struct nvmet_homa_queue, release_work);
...
static int nvmet_tcp_set_queue_sock(struct nvmet_homa_queue *queue)
{
	struct socket *sock = queue->sock;
...
#ifdef CONFIG_NVME_TARGET_TCP_TLS
static int nvmet_tcp_try_peek_pdu(struct nvmet_homa_queue *queue)
{
	struct nvme_tcp_hdr *hdr = &queue->pdu.cmd.hdr;
	int len, ret;
	struct kvec iov = {
...
static void nvmet_tcp_tls_handshake_done(void *data, int status,
					 key_serial_t peerid)
{
	struct nvmet_homa_queue *queue = data;
...
{
	struct nvmet_homa_queue *queue = container_of(to_delayed_work(w),
			struct nvmet_homa_queue, tls_handshake_tmo_work);
...
static int nvmet_tcp_tls_handshake(struct nvmet_homa_queue *queue)
{
	int ret = -EOPNOTSUPP;
...
// Call this func when a new host identified by the recv path
// For now only 1 queue so we hard-code parts of this method
		...
		struct socket *newsock)
{
	...
	// Homa use a global Homa socket to deal with this.
        //
	queue->sock = newsock;
	queue->port = port;
	queue->nr_cmds = 0;
	...
	// We are using the global socket, so no need to alloc file here.
...
// We don't need automatic queue allocation for now.
// static void nvmet_homa_new_host_work(struct work_struct *w)
// {
// 	struct nvmet_tcp_port *port =
...
static int nvmet_homa_add_port(struct nvmet_port *nport)
{
	struct nvmet_tcp_port *port;
	...
	port->nport = nport;
	INIT_WORK(&port->accept_work, nvmet_homa_new_host_work);
	if (port->nport->inline_data_size < 0)
		port->nport->inline_data_size = NVMET_TCP_DEF_INLINE_DATA_SIZE;
	// Single and global Homa socket
	ret = sock_create(port->addr.ss_family, SOCK_DGRAM,
				IPPROTO_HOMA, &port->sock);
	if (ret) {
		pr_err("failed to create a Homa socket\n");
		goto err_port;
	}
	sock_file = sock_alloc_file(queue->sock, O_CLOEXEC, NULL);
	if (IS_ERR(sock_file)) {
	...
	// TCP-specifc logic is not needed.
...
static void nvmet_homa_remove_port(struct nvmet_port *nport)
{
...
{
	struct nvmet_homa_queue *queue =
		container_of(sq, struct nvmet_homa_queue, nvme_sq);

	if (sq->qid == 0) {
		struct nvmet_homa_queue *q;
		int pending = 0;
...
static const struct nvmet_fabrics_ops nvmet_homa_ops = {
	.owner			= THIS_MODULE,
	.type			= NVMF_TRTYPE_HOMA,
	.msdbd			= 1,
	.add_port		= nvmet_homa_add_port,
	.remove_port		= nvmet_homa_remove_port,
...
static int __init nvmet_homa_init(void)
{
	int ret;

	nvmet_tcp_wq = alloc_workqueue("nvmet_homa_wq",
				WQ_MEM_RECLAIM | WQ_HIGHPRI, 0);
	if (!nvmet_tcp_wq)
		return -ENOMEM;

	ret = nvmet_register_transport(&nvmet_homa_ops);
	if (ret)
		goto err;
...
static void __exit nvmet_homa_exit(void)
{
	struct nvmet_homa_queue *queue;

	nvmet_unregister_transport(&nvmet_homa_ops);
...
module_init(nvmet_homa_init);
module_exit(nvmet_homa_exit);

MODULE_DESCRIPTION("NVMe target Homa transport driver");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("nvmet-transport-146"); /* 146 == NVMF_TRTYPE_HOMA */
